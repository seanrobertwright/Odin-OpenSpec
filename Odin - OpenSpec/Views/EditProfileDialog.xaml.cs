...
namespace Odin___OpenSpec.Views
{
    public sealed partial class EditProfileDialog : ContentDialog
    {
        ...
        public bool PhotoChanged => _photoChanged;

        public EditProfileDialog(User user)
        {
            this.InitializeComponent();
            User = user;
            _originalPhotoPath = user.PhotoPath;
            _selectedPhotoPath = user.PhotoPath;
            
            // Initialize with user data
            NameTextBox.Text = user.Name;
            ProfilePicture.DisplayName = user.Name;
            ProfilePicture.Initials = GetInitials(user.Name);
            
            // Load photo if available
            if (!string.IsNullOrEmpty(user.PhotoPath))
            {
                _ = LoadExistingPhotoAsync(user.PhotoPath);
            }
            
            UpdateValidation();
        }

        private async Task LoadExistingPhotoAsync(string photoPath)
        {
            try
            {
                if (File.Exists(photoPath))
                {
                    var file = await StorageFile.GetFileFromPathAsync(photoPath);
        ...
        private void NameTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            {
                ProfilePicture.Initials = "?";
                ProfilePicture.DisplayName = "User";
            }
        }
        ...
        private void ContentDialog_PrimaryButtonClick(ContentDialog sender, ContentDialogButtonClickEventArgs args)
        {
            ...
            // Delete old photo if it changed and was different
            if (_photoChanged && !string.IsNullOrEmpty(_originalPhotoPath) && 
                _originalPhotoPath != _selectedPhotoPath && File.Exists(_originalPhotoPath))
            {
                try
                {
        ...
        {
            // Clean up new photo if user cancels and it changed
            if (_photoChanged && !string.IsNullOrEmpty(_selectedPhotoPath) && 
                _selectedPhotoPath != _originalPhotoPath && File.Exists(_selectedPhotoPath))
            {
                try
                {
